...
##############################
def createtree_2(key,headerTable,min_support,tree_root,itemsss):
	new_headerTable={}
	#print key
	#print
	CPB=[]
	...
	for i in range(0,len(CPB)):
		if CPB[i]!=[]:
			...
			if not itemsss.has_key(cursor1.name):
				#itemsss[cursor1.name]+=support[-1]
			#else:
				itemsss[cursor1.name]=support[-1]
...
def FP_growth2(inTree,frequent,headerTable,itemsss,min_support):
	cursor=inTree
	test=0
	dict5={}
	list12=[]
	...
	if test==0:  #a single path
		...
		for element in result_1:
			...
			for e in string1:
				if dict5[e]<support:
					support=dict5[e]
			if itemsss.has_key(element+","+frequent):
				itemsss[element+","+frequent]=support+itemsss[element+","+frequent]
			else:
				itemsss[element+","+frequent]=support
	else:
		for key in headerTable:
			frequent2=key+","+frequent
			if itemsss.has_key(frequent2):
				itemsss[frequent2]=headerTable[key].occur+itemsss[frequent2]
			else:
				itemsss[frequent2]=headerTable[key].occur
			tree_root2=treeNode('Null',1,None)
			new_headerTable=createtree_2(key,headerTable,min_support,tree_root2,itemsss)
			if tree_root2.children!={}:
				FP_growth2(tree_root2,frequent2,new_headerTable,itemsss,min_support)
'''
...
frequent_pattern={}
#print headerTable
FP_growth2(tree_root,"",headerTable,frequent_pattern,min_support)
print tree
delete=[]
...
for key in frequent_pattern:
	if key[-1]==',':
		store.append(key[0:-1])
for i in store:
	frequent_pattern[i]=frequent_pattern[i+","]
	del frequent_pattern[i+',']

for key in tree:
	if frequent_pattern.has_key(key):
